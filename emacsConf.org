...
#+begin_src emacs-lisp 
  ...
  ;; diminish - hide minor modes from modeline (used by :diminish in use-package)
...
#+begin_src emacs-lisp 
  ...
  ;; Jinx - fast spell checker with multi-language support (requires libenchant)
  ;; On non-NixOS systems with Nix installed, the Nix linker can't find system libs
...
** org-download
#+begin_src emacs-lisp
  ;; org-download - drag-and-drop and paste images into org files
...
** apheleia
#+begin_src emacs-lisp
  ;;apheleia - auto-format on save (prettier, black, rustfmt, etc.)
...
** Tree-sitter
#+begin_src emacs-lisp
  ;;tree-sitter automatic grammar management
...
#+begin_src emacs-lisp
  ...
  ;; wgrep - edit grep results in-place (consult-grep → embark-export → wgrep)
  (use-package wgrep)
#+end_src
